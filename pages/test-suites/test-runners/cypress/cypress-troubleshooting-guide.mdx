...
```bash
wget https://security.ubuntu.com/ubuntu/pool/main/o/openssl/libssl1.1_1.1.1f-1ubuntu2.19_amd64.deb
...
Fortunately, you can set this manually on the command line or in your CI configuration. Follow the [instructions for running with earlier versions of Cypress](/test-suites/test-runners/cypress/cypress-faqs) to configure `RECORD_REPLAY_METADATA_FILE` (and `RECORD_ALL_CONTENT` if you’re using Firefox).
...
For more information, see [Diagnostics](/test-suites/test-runners/cypress/getting-started/diagnostic-modes) 
