...
For the example below, use `RECORD_REPLAY_API_KEY=123 node upload-failure.spec.js` to execute and record the test.
...
async function testRun() {
  try {
    ...
    const recordingId = await replayCli.viewLatestRecording({apiKey: `${process.env.RECORD_REPLAY_API_KEY}`})
