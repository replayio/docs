...
However, you can use a different CI provider by configuring your run command manually and using `@replayio/replay` to upload recordings. See [Manual CI configuration](/test-suites/test-runners/cypress/ci-environments/using-other-ci-providers) for more.
