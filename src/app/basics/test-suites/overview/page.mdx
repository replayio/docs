...
<Icon icon="error" className="mr-1.5 inline h-4 w-4 text-red-500" /> stopwatch
never shows the time `00:02:000` and still **passes** (false positive) \

<Icon icon="error" className="mr-1.5 inline h-4 w-4 text-red-500" /> stopwatch
runs too fast for `.should()` command and **fails** (false negative)
...
<Icon icon="cypress" className="mr-4 inline-block h-9 w-9" />

<Icon icon="playwright" className="mr-4 inline-block h-9 w-9" />

<Icon icon="selenium" className="mr-4 inline-block h-9 w-9" />

<Icon icon="webdriverio" className="mr-4 inline-block h-9 w-9" />

<Icon icon="puppeteer" className="mr-4 inline-block h-9 w-9" />

## FAQ

<Accordion>

<AccordionItem title="What's the difference when using Replay Browser?">
...
<AccordionItem title="When do I use Replay with my tests?">
...
<QuickLinks title="Next steps" description="Learn how to record your first Replay, manage your test suites, and debug flaky tests using Replay DevTools">
