...
For details on every available tool, see the [MCP tools reference](/basics/replay-mcp/tools).
...
The server URL is `https://dispatch.replay.io/nut/mcp`. It uses streamable HTTP. The `Authorization` header is only required for recordings that belong to a team; public recordings work without it. See [Generate an API key](/reference/ci-workflows/generate-api-key) for how to create one.
