...
`REPLAY_API_KEY` is an [app.replay.io API key](/reference/ci-workflows/generate-api-key). Each test run then uploads a recording per test, and the reporter prints the recording URLs.
...
Recordings are uploaded to the Replay Cloud where the runtime can be replayed for time-travel debugging. See [How time travel works](/basics/time-travel/how-does-time-travel-work).
