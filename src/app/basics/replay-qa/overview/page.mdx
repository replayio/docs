...
New here? Read [how Replay QA models your app](/basics/replay-qa/concepts) for the vocabulary, then [Getting good results](/basics/replay-qa/getting-good-results) for what to tell it before the first run.
...
5. **Report actionable bugs.** Each confirmed bug includes the observed behavior, evidence, and a root cause analysis that points at the responsible code.
...
If the application only exists inside a CI runner, the Replay QA CLI can open a tunnel from the runner and keep the app reachable for the complete test run. Follow the setup for [GitHub Actions](/basics/replay-qa/frpc-ci) or [CircleCI](/basics/replay-qa/circleci).
...
- **Polish passes** review each run for focused classes of issues, including accessibility, layout stability, visual glitches, network performance, React rendering, SEO, and security. See the [list of polish passes](/reference/replay-qa/polish-passes).
...
- **Bugs** collects confirmed findings with their severity, evidence, and root cause analysis.
...
Bug reports can be copied or downloaded as Markdown for a coding agent. After applying a fix, mark the bug fixed; Replay QA reruns the affected journey to verify it and reopens the bug if the problem is still there. The [bug report reference](/reference/replay-qa/bug-reports) describes each field, severity level, and status.

When a bug includes a Replay recording of the failing run, you can hand that recording to a coding agent through [Replay MCP](/basics/replay-mcp/overview) to inspect the console, code, and network traffic from the actual failure.

## Drive Replay QA from a coding agent

The whole loop is available without the web app: a coding agent can create a project, wait for QA to finish, read each bug report, apply the fix, and mark the bug fixed so QA confirms it. Use the REST API from scripts and CI, or connect Claude Code, Cursor, or another MCP client to the Replay QA MCP server.

[Driving Replay QA from a coding agent →](/basics/replay-qa/agent-integration)

To connect Replay QA to Obvious with OAuth, follow [Using Replay QA in Obvious →](/basics/replay-qa/obvious).
...
Bugs can also be filed to GitHub Issues, Linear, or Jira as they are confirmed.

[Continuous QA from GitHub →](/basics/replay-qa/github)
...
A confirmed bug includes a description of the observed behavior, reproduction steps with screenshots, expected versus actual behavior, and a root cause analysis with the evidence behind it. The underlying Replay recording preserves the browser execution that produced the finding. See [Bug reports](/reference/replay-qa/bug-reports) for the full list of fields.

</AccordionItem>

<AccordionItem title="Does Replay QA write a fix for me?">

No. The bug report identifies the root cause and the code responsible; a developer or coding agent writes the fix. Mark the bug fixed and Replay QA reruns the journey to confirm it.

</AccordionItem>

<AccordionItem title="Why did Replay QA report something that is intended behavior?">

Set the bug to invalid so it is not reported again, and add the intended behavior to the project's design document or instructions so future runs judge against it. See [Getting good results](/basics/replay-qa/getting-good-results).

</AccordionItem>

<AccordionItem title="Replay QA stopped and is asking me for something. What now?">

It pauses when it cannot continue on its own: a login it lacks, an invitation or approval the test account needs, or a project that reached its credit budget. The project chat says what it needs. [When Replay QA pauses](/basics/replay-qa/getting-good-results#when-replay-qa-pauses) lists the common cases.

</AccordionItem>

<AccordionItem title="A journey broke after I changed the UI. What should I do?">

If the flow changed on purpose, edit the journey to match; saving it schedules a rerun. If a whole area changed, start another exploration for it. See [Journey](/basics/replay-qa/concepts#journey).

</AccordionItem>

<AccordionItem title="Do I have to connect GitHub?">

No. Connect GitHub when you want QA to react to main-branch or pull-request deployments. Manual and localhost projects work without a repository connection. See [Continuous QA from GitHub](/basics/replay-qa/github).

</AccordionItem>

<AccordionItem title="What uses credits?">

Explorations, journey runs, and polish passes spend credits; each project has a budget that caps its spend. Plans and credit amounts are on the [pricing page](https://replay.io/pricing).
...
<QuickLink
  title="Test a GitHub Actions build"
  icon="pullrequest"
  href="/basics/replay-qa/frpc-ci"
  description="Keep a private pull request build reachable for Replay QA."
/>

<QuickLink
  title="Test a CircleCI build"
  icon="pullrequest"
  href="/basics/replay-qa/circleci"
  description="Run Replay QA against a private pull request build in CircleCI."
/>
