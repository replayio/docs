...
Both expose the same projects, bugs, journeys, test runs, and explorations. Pick whichever fits how your agent works. This page covers the workflow, authentication, and setup. The [OpenAPI spec](https://qa.replay.io/api/v1/openapi.json) is the complete endpoint reference.
...
Other endpoints cover journeys, explorations, test runs, credit usage, project members, GitHub pull-request runs, and app versions. The [OpenAPI spec](https://qa.replay.io/api/v1/openapi.json) lists them all with request and response shapes, and any OpenAPI-aware agent or client can load it directly.
...
The create response includes a `reverse_proxy_setup_url` you can poll for tunnel status. [Testing a localhost app](/basics/replay-qa/localhost) explains the proxy in detail, and [CI integration with FRPC](/basics/replay-qa/frpc-ci) covers builds that only exist inside GitHub Actions.
