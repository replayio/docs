...
The same example has a separate [production workflow](https://github.com/replayio/replayqa-cli-in-ci/blob/main/.github/workflows/production.yml) and [runner](https://github.com/replayio/replayqa-cli-in-ci/blob/main/scripts/run-replayqa-production.mjs). This path uses a normal Replay QA project configured with the public production URL, rather than the reverse-proxy project used for the private pull request build. Set `REPLAY_QA_API_KEY` to a Replay QA API token and create that project with the CLI:
