---
title: Uploading source maps for Replay DevTools
description: Upload source maps at build time so recordings opened in Replay DevTools show original source, even when the maps are not served publicly.
---

Recordings capture the JavaScript the browser actually ran. If that is minified production code, Replay DevTools needs source maps to show you the original files and let you add print statements in them. Uploading the maps at build time means they never have to be served from your production site.

<Callout type="note" title="Which source-map setup applies to you?">
  Uploaded source maps are used by [Replay DevTools](https://app.replay.io) for
  recordings made with the `replayio` CLI or the Playwright plugin. Replay QA
  and Replay MCP do not use uploaded maps; they read the maps your deployed app
  serves through `sourceMappingURL`. For those, see [Publishing with source
  maps](/basics/replay-qa/source-maps).
</Callout>
...
We recommend using the `REPLAY_API_KEY` environment variable to set up the API for use in all of our following examples, but each also allows passing the key directly as an argument. See [API keys and tokens](/reference/api-keys) for how to create one in your team settings.
...
With Turbopack builds, or if you prefer not to touch the webpack config, enable `productionBrowserSourceMaps` and run the CLI after `next build` instead:

```json fileName="vercel.json"
{
  "buildCommand": "next build && npx replayio upload-source-maps --group $VERCEL_GIT_COMMIT_SHA .next/static"
}
```

`.next/static` holds the client bundles; server bundles do not need to be uploaded.

## Debugging problems

Both the CLI and the Webpack plugin log each source map as it is uploaded, so you can check that the files you expect are being found. The Webpack plugin also has `logLevel: "verbose"`.

For a full trace, set `DEBUG=recordreplay:sourcemap-upload` in the environment. That output is mostly useful to send to us if uploads are failing.
