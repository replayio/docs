...
<AccordionItem title="Do I need to use `line` reporter along with `@replayio/playwright/reporter`?">

Using `line` reporter is not required. `line` reporter will print out information about playwright test run to your terminal.

</AccordionItem>

<AccordionItem title="How do I group tests ran in a matrix or across multiple runners into the same test run?">
...
<AccordionItem title="What’s in the `.replay` folder?">
...
<AccordionItem title="How do I confirm that the browsers are installed correctly">

Run `ls -alR ~/.replay/runtimes` to see the contents of the runtimes folder.

</AccordionItem>

<AccordionItem title="What other environment variables can I configure?">
